import { Message, AIProblem, FileNode } from '@/types/ai';
import { EditorTab } from '@/contexts/app-state-context';
...
function optimizedFunction(arr) {
  ...
  } else if (lowerInput.includes('generate rest api')) {
    return {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: `Okay, I will generate a REST API for you. Here are the steps and the code:
...
Step 2: Create the API routes for GET, POST, PUT, DELETE.
\`\`\`typescript
// GET /api/users
// GET /api/users/:id
// POST /api/users
...
Step 3: Show an example of how to use the API.
\`\`\`javascript
fetch('/api/users')
  .then(res => res.json())
...
REST API for users has been generated.
        `,
    }
  }
  return {
