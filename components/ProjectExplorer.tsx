...
import { useAppState, type FileNode } from "@/hooks/use-app-state"
...
export function ProjectExplorer() {
  ...
  const handleNewFile = useCallback((node: FileNode | null) => {
    setActionTargetNode(node)
    setShowNewFileDialog(true)
    setNewItemName("")
  }, [])

  const handleNewFolder = useCallback((node: FileNode | null) => {
    setActionTargetNode(node)
  ...
  }, [contextMenuNode, deleteNode, toast])
  ...
  const confirmNewFile = useCallback(() => {
    if (newItemName.trim()) {
      const parentId = actionTargetNode?.type === "folder" ? actionTargetNode.id : null // Simplified logic
      createFile(newItemName, parentId)
      setShowNewFileDialog(false)
      setNewItemName("")
    }
  }, [newItemName, createFile, actionTargetNode])
  ...
  }, [newItemName, createFolder, actionTargetNode])

  const confirmRename = useCallback(() => {
    if (newItemName.trim() && contextMenuNode) {
      renameNode(contextMenuNode.id, newItemName.trim())
      setShowRenameDialog(false)
      setNewItemName("")
    }
  }, [newItemName, contextMenuNode, renameNode])
        ...
        <div key={node.id}>
          <ContextMenu>
            ...
            <ContextMenuContent className="bg-[#252526] border-[#3c3c3c]">
              <ContextMenuItem onClick={() => handleNewFile(node)}>
                <FileText className="w-4 h-4 mr-2" />
                New File
              </ContextMenuItem>
              <ContextMenuItem onClick={() => handleNewFolder(node)}>
    ...
    <div className="h-full bg-[#252526] flex flex-col">
      ...
      <div className="p-3 border-b border-[#3c3c3c] flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="w-6 h-6 p-0" onClick={() => handleNewFile(null)}>
            <FileText className="w-3 h-3" />
          </Button>
          <Button variant="ghost" size="sm" className="w-6 h-6 p-0" onClick={() => handleNewFolder(null)}>
