...
>(({ onCodeChange }, ref) => {
  const {
    state: {
      aiAssistant: { editorTabs, activeEditorTab, showExplorer, currentProject },
    },
    updateAIAssistant,
    addEditorTab,
    updateEditorTab,
  ...
  const findFileInTree = (nodes: any[], path: string): any | null => {
  ...
  const handleSave = async () => {
    ...
    if (currentTab && currentTab.path) {
      const file = findFileInTree(currentProject?.files || [], currentTab.path)
      if (file) {
        updateFileContent(file.id, currentTab.content)
  ...
  const handleSaveAs = async (newName: string) => {
    const currentTab = editorTabs.find((tab) => tab.id === activeEditorTab)
    if (currentTab) {
      // This should create a new project, which is out of scope for this step
      toast({
        title: "Save As",
        description: `Project saved as ${newName}. Workspace will be reset.`,
      })
    }
  }
          ...
          <div className="flex-1 overflow-hidden">
            {activeEditorTab ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-gray-500">
                ...
                <Button variant="outline" className="mt-6 bg-transparent" onClick={() => insertCodeIntoEditor("", "javascript")}>
              ...
              <div className="relative mt-2 rounded-md overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => onInsertCode(message.code!.value, message.code!.language || "javascript")}
                  >
