...
import { useAppState, type FileNode, type Project } from "@/contexts/app-state-context"
...
export function ProjectExplorer() {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetNodeId: string) => {
      ...
      if (draggedNode && draggedNode !== targetNodeId) {
        moveNode(draggedNode, targetNodeId)
        toast({
          title: "Move Operation",
          description: `Moving node to ${targetNodeId}`,
        })
      }
      setDraggedNode(null)
    },
    [draggedNode, toast, moveNode],
  )
  ...
  }, [contextMenuNode, toast, deleteNode])
  ...
  const confirmNewFile = useCallback(() => {
    if (newItemName.trim()) {
      createFile(contextMenuNode?.id || null, newItemName)
      toast({
        title: "New File",
  ...
  }, [newItemName, toast, createFile, contextMenuNode])

  const confirmNewFolder = useCallback(() => {
    if (newItemName.trim()) {
      createFolder(contextMenuNode?.id || null, newItemName)
      toast({
        title: "New Folder",
  ...
  }, [newItemName, toast, createFolder, contextMenuNode])

  const confirmRename = useCallback(() => {
    if (newItemName.trim() && contextMenuNode) {
      renameNode(contextMenuNode.id, newItemName)
      toast({
        title: "Rename",
  ...
  }, [newItemName, contextMenuNode, toast, renameNode])
  ...
  const handleFolderLoad = (files: FileList) => {
    ...
    fileList.forEach((file) => {
      ...
      let currentLevel = newProject.files
      ...
      for (let i = 0; i < pathParts.length; i++) {
        ...
        if (!node) {
          node = {
            ...
            path: file.webkitRelativePath.substring(0, file.webkitRelativePath.indexOf(part) + part.length),
    ...
    updateAIAssistant({ currentProject: newProject })
  }

  if (!currentProject) {
    return (
      <div className="h-full bg-[#252526] flex flex-col items-center justify-center">
        <input
        ...
        />
        <p className="text-gray-400 text-sm mb-4">No project loaded</p>
        <Button variant="outline" className="bg-transparent border-[#3c3c3c]" onClick={fileManager.openFolder}>
